...
export default function ContractsPage() {
  // Auth state — must come first so hook order is stable before any conditional returns
  const { session, loading: authLoading } = useAuth()
  const [riskLimit, setRiskLimit] = useState<number>(0)
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : 'Gagal memuat risiko.')
    } finally {
      setLoading(false)
    }
  }, [filters])
  ...
  const loadTotal = useCallback(async () => {
    try {
      const res = await getContracts({ limit: 500 })
      setTotalRisks(res.data.length)
    } catch { /* abaikan */ }
  }, [])
  ...
  useEffect(() => {
    if (!session) return
    getMeta().then((m) => setRiskLimit(m.risk_free_limit ?? 0)).catch(() => setRiskLimit(0))
    void loadTotal()
  }, [session, loadTotal])

  // Auth gate early returns — placed after all hooks so hook order never changes between renders
  if (authLoading) {
    return (
  ...
  if (!session) return (
    <LoginGate
      title="Daftar Risiko"
      subtitleIn="Masuk untuk mengelola Daftar Risiko privat Anda."
      subtitleUp="Daftar untuk mulai mencatat risiko privat Anda."
    />
  )

  const atLimit = riskLimit > 0 && totalRisks >= riskLimit
    ...
    <div className="space-y-6">
      <section className="rounded-2xl border border-slate-800 bg-slate-900 p-6 shadow-2xl shadow-slate-950/40">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
          <div>
            <h3 className="text-2xl font-semibold text-slate-50">Daftar Risiko</h3>
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            {atLimit && (
              <p className="text-xs text-amber-300">
                Batas {riskLimit} risiko tercapai. Untuk menambah lebih banyak, jalankan versi self-hosted (tanpa batas).
              </p>
            )}
            <button
              type="button"
              ...
              disabled={atLimit}
              className={'rounded-xl border border-indigo-400 bg-indigo-500/20 px-3 py-1.5 text-xs font-semibold text-indigo-200' + (atLimit ? ' opacity-50 cursor-not-allowed' : '')}
            >
              + Risiko
            </button>
            <button
              ...
              disabled={atLimit}
              className={'rounded-xl border border-slate-700 bg-slate-800 px-3 py-1.5 text-xs font-semibold text-slate-200' + (atLimit ? ' opacity-50 cursor-not-allowed' : '')}
            >
              Import CSV
            </button>
          </div>
      ...
      {formOpen && (
        <ContractFormModal
          ...
          onSaved={() => { setFormOpen(false); void load(); void loadTotal() }}
        />
      )}
      {importOpen && (
        <ImportModal
          onClose={() => setImportOpen(false)}
          onImported={() => { setImportOpen(false); void load(); void loadTotal() }}
        />
      )}
    </div>
  )
